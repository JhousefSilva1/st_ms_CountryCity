...
public interface StCountryRepository extends JpaRepository<StCountryEntity, Long> {
    @Query("select b from StCountryEntity b where b.countryStatus = 1 order by b.idCountry asc")
    List<StCountryEntity> findAllByStatus();

    @Query("select b from StCountryEntity b where b.idCountry=?1 and b.countryStatus=?2")
